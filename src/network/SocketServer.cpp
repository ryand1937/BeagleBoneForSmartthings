...
int SocketServer::listen(){
    ...
    if(setsockopt(this->socketfd,SOL_SOCKET,SO_REUSEADDR, &one, sizeof(one)) < 0){
    	perror("Failed to set socket options(Address)");
    }
    if(setsockopt(this->socketfd,SOL_SOCKET,SO_REUSEPORT, &one, sizeof(one)) < 0){
    	perror("Failed to set socket options (Port)");
    }
    if (this->socketfd < 0){
...
bool SocketServer::isClientConnected(void)
{
